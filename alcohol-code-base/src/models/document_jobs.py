...
from models.document_analysis import DocumentAnalysis, DocumentItem


class JobsDocumentItem(DocumentItem):
    """Модель элемента накладной."""

    name: str = Field(default="Уточните название", description="Название услуги")
...
class JobsDocumentAnalysis(DocumentAnalysis):
    """Модель результата анализа услуг"""
    ...
    supplier: str | None = Field(None, description="Исполнитель")
    customer: str | None = Field(None, description="Заказчик")
    items: List[JobsDocumentItem] = Field(
...
if __name__ == "__main__":
    ...
    try:
        ...
        analysis = JobsDocumentAnalysis.model_validate(data)
